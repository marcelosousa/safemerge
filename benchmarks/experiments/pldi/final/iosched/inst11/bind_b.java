           ...
           FeedMessage message)
{
  ...
  if (TextUtils.isEmpty(feedMessage.getImageUrl()) == 0)
  {
    ...
    Glide.with(image.getContext()).load((feedMessage.getImageUrl() + "=s" + getScreenWidth(image.getContext()))).override(getScreenWidth(image.getContext()), ((int) (getScreenWidth(image.getContext()) * 9.0 / 16))).into(image);
  }
  else
  {
