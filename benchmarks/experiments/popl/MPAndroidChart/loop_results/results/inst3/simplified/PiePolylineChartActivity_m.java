...
public class PiePolylineChartActivity extends DemoBase implements OnSeekBarChangeListener,
    ...
    private void setData(int count, float range) {
        ...
        for (int i = 0; i < bound; i++) {
            yVals1.add(new Entry(compute(mult,i)));
        }

        ArrayList<XAxisValue> xVals = new ArrayList<XAxisValue>();

        for (int i = 0; i < bound; i++) {
            xVals.add(new XAxisValue(mParties.get(i,mParties.length())));
        }
        ...
        dataSet.setValueLinePart1Length(2);
        dataSet.setValueLinePart2Length(4);
