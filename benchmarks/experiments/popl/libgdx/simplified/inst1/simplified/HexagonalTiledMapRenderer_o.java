...
public class HexagonalTiledMapRenderer extends BatchTiledMapRenderer {
	...
	public int renderTileLayer (TiledMapTileLayer layer) {
		...
		if (staggerAxisX) {
			 ...
			 int row1 = Math.max(0, (int)((viewBounds.y - layerTileHeight50) / layerTileHeight));
			 int row2 = Math.min(layerHeight, (int)((viewBounds.y + viewBounds.height + layerTileHeight) / layerTileHeight));

			 int col1 = Math.max(0, (int)(((viewBounds.x - tileWidthLowerCorner) / tileWidthUpperCorner)));
			...
			for (int row = row2 - 1; row >= row1; row--) {
			 ...
			 int row1 = Math.max(0, (int)(((viewBounds.y - tileHeightLowerCorner) / tileHeightUpperCorner)));
			 int row2 = Math.min(layerHeight,
				(int)((viewBounds.y + viewBounds.height + tileHeightUpperCorner) / tileHeightUpperCorner));

			 int col1 = Math.max(0, (int)(((viewBounds.x - layerTileWidth50) / layerTileWidth)));
