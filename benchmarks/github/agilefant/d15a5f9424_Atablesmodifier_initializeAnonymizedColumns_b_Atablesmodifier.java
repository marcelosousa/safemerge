{
  try
  {
    ...
    String query = "SELECT table_name, column_name, data_type, column_key " + "FROM information_schema.columns " + "WHERE table_schema = \"" + properties.getDbName() + "\"" + "AND (data_type = \"longtext\" OR data_type = \"varchar\");";
    ...
    while (s.next())
    {
      ...
      if (tableName.equalsIgnoreCase("hourentries") && columnName.equalsIgnoreCase("DTYPE") || tableName.equalsIgnoreCase("backlogs") && columnName.equalsIgnoreCase("backlogtype") || tableName.equalsIgnoreCase("backlogs_aud") && columnName.equalsIgnoreCase("backlogtype") || tableName.equalsIgnoreCase("widgets") && columnName.equalsIgnoreCase("type"))
      {
        continue;
      }
