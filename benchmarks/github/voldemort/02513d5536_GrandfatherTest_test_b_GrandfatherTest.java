{
  ...
  try
  {
    ClientConfig config = new ClientConfig().setBootstrapUrls(("tcp://localhost:" + currentCluster.getNodeById(0).getSocketPort())).setFailureDetectorBannagePeriod(1).setMaxBootstrapRetries(10).setConnectionTimeout(5000, TimeUnit.MILLISECONDS).setSocketTimeout(5000, TimeUnit.MILLISECONDS).setRoutingTimeout(5000, TimeUnit.MILLISECONDS).setMaxConnectionsPerNode(10).setSelectors(8);
    ...
    Future<?> future = service.submit(new Runnable()
                                      {
                                        public void run ()
                                        {
                                          MigratePartitions migrate = new MigratePartitions(currentCluster, targetCluster, currentStoreDefs, targetStoreDefs, adminClient, createTempVoldemortConfig(), null, 1, false);
                                          migrate.migrate();
                                        }
                                      });
    for (int i = 0 ; i < NUM_KEYS ; i++)
    ...
    {
      config = new ClientConfig().setBootstrapUrls(("tcp://localhost:" + currentCluster.getNodeById(0).getSocketPort())).setMaxBootstrapRetries(10).setMaxConnectionsPerNode(10).setSelectors(8).setConnectionTimeout(5000, TimeUnit.MILLISECONDS).setSocketTimeout(5000, TimeUnit.MILLISECONDS).setRoutingTimeout(5000, TimeUnit.MILLISECONDS).setClientZoneId(zoneId);
      factory = new SocketStoreClientFactory(config);
      for (int storeNo = 1 ; storeNo <= numStores ; storeNo++)
      {
        storeClients[storeNo - 1] = factory.getStoreClient(("test" + storeNo));
    ...
    try
    {
      ...
      RequestRoutingType requestRoutingType = RequestRoutingType.getRequestRoutingType(false, false);
      ...
      for (int storeNo = 1 ; storeNo <= numStores ; storeNo++)
      {
        ...
        for (Node node : targetCluster.getNodes()) {
                                                     socketStoresPerNode.put(node.getId(), socketStoreFactory.create(storeDefToTest.getName(), node.getHost(), node.getSocketPort(), RequestFormatType.VOLDEMORT_V1, requestRoutingType));
                                                   }
        for (int i = 0 ; i < NUM_KEYS ; i++)
        {
