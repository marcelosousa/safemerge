class PipelineRoutedStore{ 
 void get() {
  ...
  pipeline.addEventAction(Event.CONFIGURED, new PerformParallelRequests<List<Versioned<byte[]>>, BasicPipelineData<List<Versioned<byte[]>>>>(pipelineData, (allowReadRepair ? Event.RESPONSES_RECEIVED : Event.COMPLETED), key, failureDetector, storeDef.getPreferredReads(), storeDef.getRequiredReads(), timeoutMs, nonblockingStores, nonblockingStoreRequest, Event.INSUFFICIENT_SUCCESSES, Event.INSUFFICIENT_ZONES));
