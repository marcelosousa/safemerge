{
  ...
  if (voldemortConfig.isSlopEnabled())
  {
    ...
    if (voldemortConfig.isSlopPusherJobEnabled())
    {
      ...
      scheduler.schedule("slop", (voldemortConfig.getPusherType().equals(BlockingSlopPusherJob.TYPE_NAME) ? new BlockingSlopPusherJob(storeRepository, metadata, failureDetector, voldemortConfig, scanPermitWrapper) : new StreamingSlopPusherJob(storeRepository, metadata, slopStreamingFailureDetector, voldemortConfig, scanPermitWrapper)), nextRun, voldemortConfig.getSlopFrequencyMs());
    }
