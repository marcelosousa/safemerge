{
  ...
  parser.accepts("interval", "print requests on this interval, -1 disables").withRequiredArg().ofType(Integer.class);
  ...
  ClientConfig clientConfig = new ClientConfig().setMaxThreads(numThreads).setMaxTotalConnections(numThreads).setMaxConnectionsPerNode(numThreads).setBootstrapUrls(url).setConnectionTimeout(60, TimeUnit.SECONDS).setSocketTimeout(60, TimeUnit.SECONDS).setSocketBufferSize((4 * 1024)).setEnablePipelineRoutedStore(options.has("pipeline-routed-store"));
