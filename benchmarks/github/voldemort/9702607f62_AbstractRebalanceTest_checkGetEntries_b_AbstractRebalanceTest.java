class AbstractRebalanceTest{ 
 void checkGetEntries() {
  ...
  Store<ByteArray, byte[]> storeRW = getSocketStore(testStoreNameRW, node.getHost(), node.getSocketPort());
  Store<ByteArray, byte[]> storeRO = getSocketStore(testStoreNameRO, node.getHost(), node.getSocketPort());
