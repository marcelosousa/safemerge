class ReadOnlyStorePerformanceTest{ 
 void main() {
  ...
  final Store<ByteArray, byte[]> store = new ReadOnlyStorageEngine("test", searcher, new RoutingStrategyFactory().updateRoutingStrategy(storeDef, cluster), nodeId, new File(storeDir), 0);
