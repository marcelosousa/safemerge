class RedirectingStoreTest{ 
 void testProxyGetAll() {
  ...
  incrementVersionAndPut(server0.getMetadataStore(), MetadataStore.REBALANCING_STEAL_INFO, new RebalancerState(Arrays.asList(new RebalancePartitionsInfo(0, 1, Arrays.asList(1), new ArrayList<Integer>(0), new ArrayList<Integer>(0), Arrays.asList(testStoreName), new HashMap<String, String>(), new HashMap<String, String>(), 0))));
  checkGetAllEntries(entryMap, server0, getRedirectingStore(server0.getMetadataStore(), testStoreName), Arrays.asList(1));
}
}