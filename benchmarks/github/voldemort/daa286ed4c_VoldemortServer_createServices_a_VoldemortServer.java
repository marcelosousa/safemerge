{
  ...
  if (voldemortConfig.isHttpServerEnabled())
    services.add(new HttpService(this, storeRepository, RequestFormatType.VOLDEMORT_V1, voldemortConfig.getMaxThreads(), identityNode.getHttpPort()));
  if (voldemortConfig.isSocketServerEnabled())
  {
    RequestHandlerFactory socketRequestHandlerFactory = new SocketRequestHandlerFactory(this.storeRepository, this.metadata, this.voldemortConfig, this.asyncRunner, null);
  ...
  if (voldemortConfig.isAdminServerEnabled())
  {
    ...
    SocketRequestHandlerFactory adminRequestHandlerFactory = new SocketRequestHandlerFactory(this.storeRepository, this.metadata, this.voldemortConfig, this.asyncRunner, rebalancer);
    if (voldemortConfig.getUseNioConnector())
    {
      logger.info("Using NIO Connector for Admin Service.");
      services.add(new NioSocketService(adminRequestHandlerFactory, identityNode.getAdminPort(), voldemortConfig.getAdminSocketBufferSize(), voldemortConfig.getNioConnectorSelectors(), "admin-server", voldemortConfig.isJmxEnabled()));
    }
    else
    {
